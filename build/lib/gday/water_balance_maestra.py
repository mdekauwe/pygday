# coding: utf-8
"""
Link against MAESTRA to do the transpiration calculations...
    ...
    * McMurtrie, R. (1990) Water/nutrient interactions affecting the
    ...
    def calculate_water_balance(self, day, daylen):
        ...
        if self.control.trans_model == 0:
            ...
            self.calc_transpiration_maestra(vpd, net_rad, leaf_temp, wind,
                                                ca, daylen, press)
    ...
    def calc_transpiration_maestra(self, vpd, net_rad, tavg, wind, ca,
                                        daylen, press):
        
        
        # zero plan displacement height [m]
        d = self.params.displace_ratio * self.params.canht
        z0 = self.params.dz0v_dh * self.params.canht
        
        
        
        slope = (utils.satur(tavg + 0.1) - utils.satur(tavg)) / 0.1
        gb = physiol.gbcan(wind, self.params.canht, z0, d, press, tavg)
        
        wleaf = 0.0001
        # Boundary layer conductance for heat - single sided, forced convection
        gbhu = physiol.gbhforced(tavg, press, wind, wleaf)
        print gbhu
        
        # Boundary layer conductance for heat - single sided, free convection
        TLEAF = tavg - 2.0
        gbhu = physiol.gbhfree(tavg, TLEAF, press, wleaf)
        print gbhu
        sys.exit()
        ...
        slope = P.calc_slope_of_saturation_vapour_pressure_curve(tavg)
        
        #print slope*1000.0 # kilopascals to pa
...
class PenmanMonteith(object):
    ...
    * Dawes and Zhang (2011) Waves - An integrated energy and water balance model
    ...
    def calc_evaporation(self, vpd, wind_speed, gs, net_rad,
        ...
        ga = self.calc_atmos_boundary_layer_conductance(wind_speed)
    ...
    def calc_atmos_boundary_layer_conductance(self, wind_speed):
